...
const Bubble = (props:any) => {
  ...
  const [x_axis,setX_axis] = useState(controls?.x ? controls.x : undefined);
  const [y_axis,setY_axis] = useState(controls?.y ? controls.y : undefined);
  ...
  const savedMetrics = controls.x.savedMetrics;
  const savedMetricNames = savedMetrics.map((item: { metric_name: any; }) => item.metric_name);  
      ...
      {x_axis !== undefined && (
        <div className='dropdown-container'>
          ...
          <select id="dropdown" value={x_axis} onChange={handleChangeForX}>
            {savedMetricNames.map((name: string, index: number) => (
              <option key={index} value={name}>
                {name}
              </option>
            ))}
